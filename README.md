...
Após executar a função de treinamento, o modelo convergiu em **29** épocas com uma perda média de **0.0082** e  **Accuracy** = **99,7%** conforme a linha abaixo.
...
Através da matriz de confusão e das métrica na **Figura 4**, podemos ter mais informações sobre o desempenho do modelo de classificação em questão.  O modelo **MLP**  uma **Accuracy** = **99,7%.** O modelo classificou corretamente **462** das **464** instâncias **não spam** com **Precision = 99,5%** e errou **1** das **436** instâncias **spam** com **Recall = 99,8%**.
...
Através do **LIME**, é possível inspecionar localmente as instâncias para entender quais features exerceram mais influência nos erros de predição por exemplo.  Na **Figura 5**, a amostra analisada é um falso negativo, algo indesejado quando se trata de segurança.
...
O modelo de  Regressão Logística obteve  uma **Accuracy** = **99,7%.** O modelo classificou corretamente **464** das **464** instâncias **não spam** com **Precision = 100%** porém, errou **3** das **436** instâncias **spam** com **Recall = 99,3%**.

![matriz.jpg](Avaliac%CC%A7a%CC%83o%20te%CC%81cnica%20Axur%200bee9d675d504ce5ab3ebd738bf0adb0/matriz%201.jpg)

                                          **Figura 5: Matriz de confusão e métricas de classificação Regressão.**

Na **Figura 6**, os termos **15, you, to, code, sent e with** estão contribuindo para o modelo classificar a instância  como **não spam** e os termos **http, com e link** para classificar como **spam**.
...
Foi treinado um modelo **Multi Layer Perceptron** usando frameworks como **Keras** e **TensorFlow** para classificar dados de **SMS** do dataset **train_data.** Após vários testes, o modelo atingiu um bom resultado mostrando ser aplicável em dados reais. O código e os resultados desse modelo estão disponíveis no arquivo **mlp.ipynb.**

A título de comparação, foi treinado um modelo de **Regressão Logística** usando o **scikit-learn,** adotando os mesmos critérios de **Preparação dos Dados aplicado** a ****MLP**.** O código e os resultados obtidos por esse modelo estão disponíveis no arquivo **regressão_logistica.ipynb.**

Apesar do modelo de **Regressão Logística** ser mais simples, os resultados obtidos foram satisfatórios com um desempenho semelhante ao modelo **MLP,** onde ambos atigiram **Acuracy = 99,7%,** porém com um maior número de falsos negativos, com **Recall = 99,3%** contra **99,8%** alcançados pela **MLP**.

Essa diferença de **Recall** deve ser levada em consideração na escolha dentre os modelos, principalmente em um cenário de segurança onde erros de classificação podem causar prejuízos.
